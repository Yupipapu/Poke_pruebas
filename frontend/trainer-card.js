...
// Función para cargar el sprite del Pokémon en el canvas probando respaldos automáticos
async function loadPokemonSpriteWithFallback(id) {
  const primaryUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/refs/heads/master/sprites/pokemon/versions/generation-ix/champions/${id}.png`;
  const auxUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/refs/heads/master/sprites/pokemon/versions/generation-ix/scarlet-violet/${id}.png`;
  const defaultUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`;

  try {
    return await loadImage(primaryUrl);
  } catch (e1) {
    try {
      return await loadImage(auxUrl);
    } catch (e2) {
      try {
        return await loadImage(defaultUrl);
      } catch (e3) {
        throw new Error("No se pudo cargar el sprite");
      }
    }
  }
}
...
async function renderTrainerCardCanvas() {
  ...
  if (currentCardTrainer.pokemon) {
    for (let i = 0; i < currentCardTrainer.pokemon.length; i++) {
      ...
      // 1. Cargar imagen con respaldo automático (Champions -> Scarlet/Violet -> Default)
      try {
        const pokeImg = await loadPokemonSpriteWithFallback(p.id);
        ctx.drawImage(pokeImg, x, y, pokeSize, pokeSize);
      } catch (e) { }

      // 2. Posiciones independientes para los nombres por fila
      const nameXOffset = 65;
      const nameYOffsetRow1 = 150;
      const nameYOffsetRow2 = 145;
